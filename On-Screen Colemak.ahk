...
k_FontSize = 10	; Changing this will grow/shrink the entire on-screen keyboard!
...
k_AddRow(k_RowLeft, k_RowMiddle, k_RowRight)
{
	...
	{
		If (A_LoopField = "t" or A_LoopField = "n") {
			Gui Font, s%k_FontSize% %k_FontStyle% Underline, %k_FontName%
			Gui Add, Button, %k_Position%, %A_LoopField%
			Gui Font, s%k_FontSize% Norm %k_FontStyle%, %k_FontName%
		} else {
			Gui Add, Button, %k_Position%, %A_LoopField%
		}
	}
